...
public class GuiExportScreen extends GuiScreen {
            ...
            if (item != null) {
                if (item.getHasSubtypes()) {
                    if (item instanceof ItemSlab) {
                        for (int j = 0; j < BlockStep.field_22037_a.length; j++) {
                            this.allItems.add(new ItemStack(item, 1, j));
                        }
                    } else if (item instanceof ItemDye) {
                        for (int j = 0; j < 16; j++) {
                            this.allItems.add(new ItemStack(item, 1, j));
                        }
                    } else if (item instanceof ItemLog) {
                        for (int j = 0; j < 3; j++) {
                            this.allItems.add(new ItemStack(item, 1, j));
                        }
                    } else if (item instanceof ItemSapling) {
                        for (int j = 0; j < 3; j++) {
                            this.allItems.add(new ItemStack(item, 1, j));
                        }
                    } else {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (displayIndex >= 0 && displayIndex < itemsToExport.size()) {
            ...
            String itemName = stack.getItem().getItemNameIS(stack);
